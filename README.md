...
| カテゴリ | 主な内容 |
|---------|---------|
| 🪟 **UI・レイアウト** | - アプリタイトルを `TAS` に設定<br>- 自動リサイズ・フォントスケーリング<br>- ステータスバーで状態表示 |
| 🔌 **シリアル通信** | - `QSerialPort` による Arduino UNO R4 接続<br>- ポート自動列挙<br>- ボーレート設定（115200） |
| 🌡️ **温度表示** | - MAX6675 から受信した温度をリアルタイム表示<br>- ℃表示、エラーチェックあり |
| 📝 **ロギング** | - `.csv` 保存<br>- タイムスタンプ付き温度ログ<br>- ログファイル自動生成 |
| 📈 **グラフ表示（任意）** | - QtCharts によるリアルタイムプロット（必要時追加可能） |
